...
object Top10MostPopularSuperhero {
  ...
  def main(args: Array[String]) {
    ...
    val topTenMostPopular = mostPopular.take(10)


    for (hero <- topTenMostPopular) {
        val heroId = hero._2
        val numOfFriends = hero._1

      // Look up the name (lookup returns an array of results, so we need to access the first result with (0)).
        val heroName = namesRdd.lookup(heroId)(0)

      // Print out our answer!
      println(s"$heroName is one of the top 10 most popular superhero with ${numOfFriends} co-appearances.")
